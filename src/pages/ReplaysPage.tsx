...
export default function ReplaysPage() {
  return (<div style={{padding:"var(--space-8)",maxWidth:1000}}>
    <h1 style={{fontSize:"var(--font-size-2xl)",fontWeight:700,marginBottom:"var(--space-6)",display:"flex",alignItems:"center",gap:"var(--space-2)"}}><PlayCircle style={{color:"var(--color-accent-primary)"}}/> Replay History</h1>
    <div className="card">
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        ...
        <tbody>
          {replays.map(r=>(<tr key={r.id} style={{borderBottom:"1px solid var(--color-border)"}}>
            ...
            <td style={{padding:"12px 16px"}}><span className={`badge ${r.status===200?"badge-green":"badge-red"}`}>{r.status}</span></td>
