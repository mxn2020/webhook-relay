  ...
  return (<div style={{ display: "flex", height: "100vh" }}>
    <div style={{ width: 350, borderRight: "1px solid var(--color-border)", display: "flex", flexDirection: "column", background: "var(--color-bg-secondary)" }}>
      <div style={{ padding: "var(--space-4)", borderBottom: "1px solid var(--color-border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h2 style={{ fontSize: "14px", fontWeight: 700, textTransform: "uppercase", display: "flex", alignItems: "center", gap: "var(--space-2)", margin: 0 }}><Inbox size={16} /> Requests</h2>
        <div style={{ display: "flex", gap: "var(--space-1)" }}>
          <Button variant="ghost" size="sm" icon={<RefreshCw size={14} />} style={{ padding: 4 }} />
          <Button variant="ghost" size="sm" icon={<Trash2 size={14} />} style={{ padding: 4, color: "var(--color-error)" }} />
        </div>
      </div>
      <div style={{ flex: 1, overflowY: "auto" }}>
        {requests.map((r, i) => (<div key={r.id} style={{ padding: "var(--space-3) var(--space-4)", borderBottom: "1px solid var(--color-border)", background: i === 0 ? "var(--color-bg-hover)" : "transparent", cursor: "pointer", borderLeft: i === 0 ? "3px solid var(--color-accent-primary)" : "3px solid transparent" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <Badge variant={r.status === 200 ? "success" : "danger"}>{r.status} {r.method}</Badge>
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "var(--space-6)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "var(--space-6)" }}>
        <div>
          <h1 className="mono" style={{ fontSize: "var(--font-size-xl)", fontWeight: 700, margin: '0 0 4px 0' }}>/hooks/stripe</h1>
          <div style={{ fontSize: "13px", color: "var(--color-text-secondary)" }}>req_1 • 14:23:45 • Passed 1 forwarding rule</div>
        </div>
        <Button variant="primary" icon={<RefreshCw size={16} />}>Replay</Button>
      </div>
      ...
      <h3 style={{ fontSize: "14px", fontWeight: 700, textTransform: "uppercase", marginBottom: "var(--space-3)", marginTop: 0 }}>Headers</h3>
      ...
      <h3 style={{ fontSize: "14px", fontWeight: 700, textTransform: "uppercase", marginBottom: "var(--space-3)", marginTop: 0 }}>JSON Payload</h3>
